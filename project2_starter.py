    ...
    with open(html_path, 'r', encoding="utf-8-sig") as file:
        ...
        for link in links:
            ...
            if href and "/rooms/" in href:
                listing_id =href.split("/rooms/")[1]
                title= link.get_text().strip()
                if title and listing_id not in seen_ids:
...
def get_listing_details(listing_id) -> dict:
    ...
    with open(file_path, 'r', encoding="utf-8-sig") as file:
        ...
        match= re.search(r"STR-\d{7}|20\d{2}-00\d{4}STR", text)
        if match:
        ...
        if "Superhost" in text:
            ...
            host_type="regular"
        host_name=""
        host_name_find= soup.find(string=re.compile("Hosted by"))
        if host_name_find:
            host_name= host_name_find.split("Hosted by")[-1].strip()
        room_data=""
        h2=soup.find("h2")
        if h2:
            room_data= h2.get_text().strip()
        if "Private" in room_data:
            room_type="Private Room"
        elif "Shared" in room_data:
            room_type="Shared Room"
        else:
            room_type="Entire Room"
        location_rating=0.0
        rating=re.search(r"Location\s*([\d.]+)", text)
        if rating:
            location_rating =float(rating.group(1))
        return {
            listing_id: {
...
class TestCases(unittest.TestCase):
    ...
    def test_create_listing_database(self):
        ...
        self.assertEqual(self.detailed_date[-1], ("Guest suite in Mission District", "467507", "STR-0005349", "Superhost", "Jennifer", "Entire Room", 4.8))
    ...
    def test_output_csv(self):
        ...
        rows=[]
        with open(out_path, 'r', encoding="utf-8-sig") as csvfile:
